package config

import (
	"fmt"
	"os"
	"strconv"
	"strings"
	"time"
)

func ApplyEnvOverrides(cfg *Config) error {
	var errors []string
	setString(&cfg.Server.Addr, "GONGKAN_SERVER_ADDR")
	setString(&cfg.Database.DSN, "GONGKAN_DATABASE_DSN")
	setInt32(&cfg.Database.MaxOpenConns, "GONGKAN_DATABASE_MAX_OPEN_CONNS", &errors)
	setInt32(&cfg.Database.MaxIdleConns, "GONGKAN_DATABASE_MAX_IDLE_CONNS", &errors)
	setDuration(&cfg.Database.MaxConnLifetime, "GONGKAN_DATABASE_MAX_CONN_LIFETIME", &errors)
	setString(&cfg.Redis.Addr, "GONGKAN_REDIS_ADDR")
	setString(&cfg.Redis.Password, "GONGKAN_REDIS_PASSWORD")
	setInt(&cfg.Redis.DB, "GONGKAN_REDIS_DB", &errors)
	setString(&cfg.Storage.Type, "GONGKAN_STORAGE_TYPE")
	setString(&cfg.Storage.LocalDir, "GONGKAN_STORAGE_LOCAL_DIR")
	setString(&cfg.Storage.MinIO.Endpoint, "GONGKAN_MINIO_ENDPOINT")
	setString(&cfg.Storage.MinIO.AccessKey, "GONGKAN_MINIO_ACCESS_KEY")
	setString(&cfg.Storage.MinIO.SecretKey, "GONGKAN_MINIO_SECRET_KEY")
	setString(&cfg.Storage.MinIO.Bucket, "GONGKAN_MINIO_BUCKET")
	setBool(&cfg.Storage.MinIO.UseSSL, "GONGKAN_MINIO_USE_SSL", &errors)
	setString(&cfg.Auth.JWTSecretEnv, "GONGKAN_JWT_SECRET_ENV")
	setString(&cfg.Auth.JWTSecretEnv, "GONGKAN_AUTH_JWT_SECRET_ENV")
	setBool(&cfg.Auth.BootstrapAdmin.Enabled, "GONGKAN_BOOTSTRAP_ADMIN_ENABLED", &errors)
	setString(&cfg.Auth.BootstrapAdmin.Username, "GONGKAN_BOOTSTRAP_ADMIN_USERNAME")
	setString(&cfg.Auth.BootstrapAdmin.PasswordEnv, "GONGKAN_BOOTSTRAP_ADMIN_PASSWORD_ENV")
	setByteSize(&cfg.Files.MaxUploadSize, "GONGKAN_FILES_MAX_UPLOAD_SIZE", &errors)
	setString(&cfg.Files.TempDir, "GONGKAN_FILES_TEMP_DIR")
	setStringSlice(&cfg.Files.AllowedExtensions, "GONGKAN_FILES_ALLOWED_EXTENSIONS")
	setStringSlice(&cfg.Files.AllowedMIMETypes, "GONGKAN_FILES_ALLOWED_MIME_TYPES")
	setBool(&cfg.Files.DeleteObjectOnSoftDelete, "GONGKAN_FILES_DELETE_OBJECT_ON_SOFT_DELETE", &errors)
	setString(&cfg.Artifacts.DownloadMode, "GONGKAN_ARTIFACTS_DOWNLOAD_MODE")
	setBool(&cfg.Artifacts.AllowPresignDownload, "GONGKAN_ARTIFACTS_ALLOW_PRESIGN_DOWNLOAD", &errors)
	setDuration(&cfg.Artifacts.DefaultPresignTTL, "GONGKAN_ARTIFACTS_DEFAULT_PRESIGN_TTL", &errors)
	setString(&cfg.Python.Executable, "GONGKAN_PYTHON_EXECUTABLE")
	setString(&cfg.Python.ProjectDir, "GONGKAN_PYTHON_PROJECT_DIR")
	setString(&cfg.Python.ConfigPath, "GONGKAN_PYTHON_CONFIG_PATH")
	setDuration(&cfg.Python.DefaultTimeout, "GONGKAN_PYTHON_DEFAULT_TIMEOUT", &errors)
	setBool(&cfg.Python.ArtifactValidationEnabled, "GONGKAN_PYTHON_ARTIFACT_VALIDATION_ENABLED", &errors)
	setDuration(&cfg.Python.KillGracePeriod, "GONGKAN_PYTHON_KILL_GRACE_PERIOD", &errors)
	setInt64(&cfg.Python.StdoutLogMaxBytes, "GONGKAN_PYTHON_STDOUT_LOG_MAX_BYTES", &errors)
	setInt64(&cfg.Python.StderrLogMaxBytes, "GONGKAN_PYTHON_STDERR_LOG_MAX_BYTES", &errors)
	setString(&cfg.Python.Step15DefaultRetrievalMode, "GONGKAN_PYTHON_STEP15_DEFAULT_RETRIEVAL_MODE")
	setString(&cfg.Python.Step15DefaultPromptVersion, "GONGKAN_PYTHON_STEP15_DEFAULT_PROMPT_VERSION")
	setString(&cfg.Python.Step15DefaultRows, "GONGKAN_PYTHON_STEP15_DEFAULT_ROWS")
	setBool(&cfg.Python.IngestCommandEnabled, "GONGKAN_PYTHON_INGEST_COMMAND_ENABLED", &errors)
	setInt(&cfg.Jobs.FillConcurrency, "GONGKAN_JOBS_FILL_CONCURRENCY", &errors)
	setInt(&cfg.Jobs.IngestionConcurrency, "GONGKAN_JOBS_INGESTION_CONCURRENCY", &errors)
	setInt(&cfg.Jobs.MaxPythonProcesses, "GONGKAN_JOBS_MAX_PYTHON_PROCESSES", &errors)
	setString(&cfg.Jobs.RedisNamespace, "GONGKAN_JOBS_REDIS_NAMESPACE")
	setInt(&cfg.Jobs.WorkerConcurrency, "GONGKAN_JOBS_WORKER_CONCURRENCY", &errors)
	setDuration(&cfg.Jobs.DefaultTimeout, "GONGKAN_JOBS_DEFAULT_TIMEOUT", &errors)
	setInt(&cfg.Jobs.MaxAttempts, "GONGKAN_JOBS_MAX_ATTEMPTS", &errors)
	setDuration(&cfg.Jobs.RetryBackoff, "GONGKAN_JOBS_RETRY_BACKOFF", &errors)
	setDuration(&cfg.Jobs.HeartbeatInterval, "GONGKAN_JOBS_HEARTBEAT_INTERVAL", &errors)
	setInt(&cfg.Jobs.EventBufferSize, "GONGKAN_JOBS_EVENT_BUFFER_SIZE", &errors)
	setBool(&cfg.Jobs.EnableNoopJob, "GONGKAN_JOBS_ENABLE_NOOP_JOB", &errors)
	setBool(&cfg.Jobs.EventBusEnabled, "GONGKAN_JOBS_EVENT_BUS_ENABLED", &errors)
	setString(&cfg.Jobs.EventChannel, "GONGKAN_JOBS_EVENT_CHANNEL")
	setStringSlice(&cfg.CORS.AllowedOrigins, "GONGKAN_CORS_ALLOWED_ORIGINS")
	setStringSlice(&cfg.CORS.AllowedMethods, "GONGKAN_CORS_ALLOWED_METHODS")
	setStringSlice(&cfg.CORS.AllowedHeaders, "GONGKAN_CORS_ALLOWED_HEADERS")
	setString(&cfg.Logging.Level, "GONGKAN_LOGGING_LEVEL")
	setString(&cfg.Logging.Encoding, "GONGKAN_LOGGING_ENCODING")
	setBool(&cfg.Logging.Development, "GONGKAN_LOGGING_DEVELOPMENT", &errors)
	setBool(&cfg.Observability.MetricsEnabled, "GONGKAN_OBSERVABILITY_METRICS_ENABLED", &errors)
	setBool(&cfg.Observability.PprofEnabled, "GONGKAN_OBSERVABILITY_PPROF_ENABLED", &errors)
	setString(&cfg.Observability.PprofAddr, "GONGKAN_OBSERVABILITY_PPROF_ADDR")
	setBool(&cfg.Observability.TracingEnabled, "GONGKAN_OBSERVABILITY_TRACING_ENABLED", &errors)
	setString(&cfg.Observability.TracingExporter, "GONGKAN_OBSERVABILITY_TRACING_EXPORTER")
	setString(&cfg.Observability.OTLPEndpoint, "GONGKAN_OBSERVABILITY_OTLP_ENDPOINT")
	setBool(&cfg.Security.RateLimitEnabled, "GONGKAN_SECURITY_RATE_LIMIT_ENABLED", &errors)
	setInt(&cfg.Security.RateLimitRPS, "GONGKAN_SECURITY_RATE_LIMIT_RPS", &errors)
	setInt(&cfg.Security.RateLimitBurst, "GONGKAN_SECURITY_RATE_LIMIT_BURST", &errors)
	setByteSize(&cfg.Security.MaxBodySize, "GONGKAN_SECURITY_MAX_BODY_SIZE", &errors)
	setBool(&cfg.Security.HSTSEnabled, "GONGKAN_SECURITY_HSTS_ENABLED", &errors)
	setDuration(&cfg.Operations.GracefulShutdownTimeout, "GONGKAN_OPERATIONS_GRACEFUL_SHUTDOWN_TIMEOUT", &errors)
	setBool(&cfg.ModelGateway.Enabled, "MODEL_GATEWAY_ENABLED", &errors)
	setBool(&cfg.ModelGateway.Enabled, "GONGKAN_MODEL_GATEWAY_ENABLED", &errors)
	setBool(&cfg.ModelGateway.BindToAPI, "GONGKAN_MODEL_GATEWAY_BIND_TO_API", &errors)
	setString(&cfg.ModelGateway.InternalBaseURL, "GONGKAN_MODEL_GATEWAY_INTERNAL_BASE_URL")
	setString(&cfg.ModelGateway.InternalBaseURL, "NDR_MODEL_GATEWAY_BASE_URL")
	setBool(&cfg.ModelGateway.RequireInternalToken, "GONGKAN_MODEL_GATEWAY_REQUIRE_INTERNAL_TOKEN", &errors)
	setString(&cfg.ModelGateway.InternalTokenEnv, "GONGKAN_MODEL_GATEWAY_INTERNAL_TOKEN_ENV")
	setInt(&cfg.ModelGateway.Defaults.RequestTimeoutSeconds, "GONGKAN_MODEL_GATEWAY_REQUEST_TIMEOUT_SECONDS", &errors)
	setInt(&cfg.ModelGateway.Defaults.QueueTimeoutSeconds, "GONGKAN_MODEL_GATEWAY_QUEUE_TIMEOUT_SECONDS", &errors)
	setInt64(&cfg.ModelGateway.Defaults.MaxRequestBodyBytes, "GONGKAN_MODEL_GATEWAY_MAX_REQUEST_BODY_BYTES", &errors)
	setInt64(&cfg.ModelGateway.Defaults.MaxResponseBodyBytes, "GONGKAN_MODEL_GATEWAY_MAX_RESPONSE_BODY_BYTES", &errors)
	setInt(&cfg.ModelGateway.Defaults.RetryMaxAttempts, "GONGKAN_MODEL_GATEWAY_RETRY_MAX_ATTEMPTS", &errors)
	setInt(&cfg.ModelGateway.Defaults.RetryBaseDelayMillis, "GONGKAN_MODEL_GATEWAY_RETRY_BASE_DELAY_MS", &errors)
	setInt(&cfg.ModelGateway.Defaults.RetryMaxDelayMillis, "GONGKAN_MODEL_GATEWAY_RETRY_MAX_DELAY_MS", &errors)
	setInt(&cfg.ModelGateway.Defaults.CircuitFailureThreshold, "GONGKAN_MODEL_GATEWAY_CIRCUIT_FAILURE_THRESHOLD", &errors)
	setInt(&cfg.ModelGateway.Defaults.CircuitOpenSeconds, "GONGKAN_MODEL_GATEWAY_CIRCUIT_OPEN_SECONDS", &errors)
	setString(&cfg.ModelGateway.Chat.UpstreamURL, "GONGKAN_MODEL_GATEWAY_CHAT_UPSTREAM_URL")
	setString(&cfg.ModelGateway.Chat.APIKeyEnv, "GONGKAN_MODEL_GATEWAY_CHAT_API_KEY_ENV")
	setInt(&cfg.ModelGateway.Chat.MaxConcurrency, "GONGKAN_MODEL_GATEWAY_CHAT_MAX_CONCURRENCY", &errors)
	setInt(&cfg.ModelGateway.Chat.MaxQueueSize, "GONGKAN_MODEL_GATEWAY_CHAT_MAX_QUEUE_SIZE", &errors)
	setInt(&cfg.ModelGateway.Chat.QPS, "GONGKAN_MODEL_GATEWAY_CHAT_QPS", &errors)
	setInt(&cfg.ModelGateway.Chat.RPM, "GONGKAN_MODEL_GATEWAY_CHAT_RPM", &errors)
	setInt(&cfg.ModelGateway.Chat.PerRunMaxInflight, "GONGKAN_MODEL_GATEWAY_CHAT_PER_RUN_MAX_INFLIGHT", &errors)
	setInt(&cfg.ModelGateway.Chat.TimeoutSeconds, "GONGKAN_MODEL_GATEWAY_CHAT_TIMEOUT_SECONDS", &errors)
	setString(&cfg.ModelGateway.Embedding.UpstreamURL, "GONGKAN_MODEL_GATEWAY_EMBEDDING_UPSTREAM_URL")
	setString(&cfg.ModelGateway.Embedding.APIKeyEnv, "GONGKAN_MODEL_GATEWAY_EMBEDDING_API_KEY_ENV")
	setInt(&cfg.ModelGateway.Embedding.MaxConcurrency, "GONGKAN_MODEL_GATEWAY_EMBEDDING_MAX_CONCURRENCY", &errors)
	setInt(&cfg.ModelGateway.Embedding.MaxQueueSize, "GONGKAN_MODEL_GATEWAY_EMBEDDING_MAX_QUEUE_SIZE", &errors)
	setInt(&cfg.ModelGateway.Embedding.QPS, "GONGKAN_MODEL_GATEWAY_EMBEDDING_QPS", &errors)
	setInt(&cfg.ModelGateway.Embedding.RPM, "GONGKAN_MODEL_GATEWAY_EMBEDDING_RPM", &errors)
	setInt(&cfg.ModelGateway.Embedding.PerRunMaxInflight, "GONGKAN_MODEL_GATEWAY_EMBEDDING_PER_RUN_MAX_INFLIGHT", &errors)
	setInt(&cfg.ModelGateway.Embedding.TimeoutSeconds, "GONGKAN_MODEL_GATEWAY_EMBEDDING_TIMEOUT_SECONDS", &errors)
	setString(&cfg.ModelGateway.Rerank.UpstreamURL, "GONGKAN_MODEL_GATEWAY_RERANK_UPSTREAM_URL")
	setString(&cfg.ModelGateway.Rerank.APIKeyEnv, "GONGKAN_MODEL_GATEWAY_RERANK_API_KEY_ENV")
	setInt(&cfg.ModelGateway.Rerank.MaxConcurrency, "GONGKAN_MODEL_GATEWAY_RERANK_MAX_CONCURRENCY", &errors)
	setInt(&cfg.ModelGateway.Rerank.MaxQueueSize, "GONGKAN_MODEL_GATEWAY_RERANK_MAX_QUEUE_SIZE", &errors)
	setInt(&cfg.ModelGateway.Rerank.QPS, "GONGKAN_MODEL_GATEWAY_RERANK_QPS", &errors)
	setInt(&cfg.ModelGateway.Rerank.RPM, "GONGKAN_MODEL_GATEWAY_RERANK_RPM", &errors)
	setInt(&cfg.ModelGateway.Rerank.PerRunMaxInflight, "GONGKAN_MODEL_GATEWAY_RERANK_PER_RUN_MAX_INFLIGHT", &errors)
	setInt(&cfg.ModelGateway.Rerank.TimeoutSeconds, "GONGKAN_MODEL_GATEWAY_RERANK_TIMEOUT_SECONDS", &errors)
	if len(errors) > 0 {
		return fmt.Errorf("apply environment overrides: %s", strings.Join(errors, "; "))
	}
	return nil
}

func setString(target *string, name string) {
	if value, ok := os.LookupEnv(name); ok {
		*target = value
	}
}

func setInt(target *int, name string, errors *[]string) {
	if value, ok := os.LookupEnv(name); ok {
		parsed, err := strconv.Atoi(value)
		if err != nil {
			*errors = append(*errors, fmt.Sprintf("%s must be an integer", name))
			return
		}
		*target = parsed
	}
}

func setInt32(target *int32, name string, errors *[]string) {
	if value, ok := os.LookupEnv(name); ok {
		parsed, err := strconv.ParseInt(value, 10, 32)
		if err != nil {
			*errors = append(*errors, fmt.Sprintf("%s must be an int32", name))
			return
		}
		*target = int32(parsed)
	}
}

func setInt64(target *int64, name string, errors *[]string) {
	if value, ok := os.LookupEnv(name); ok {
		parsed, err := strconv.ParseInt(value, 10, 64)
		if err != nil {
			*errors = append(*errors, fmt.Sprintf("%s must be an int64", name))
			return
		}
		*target = parsed
	}
}

func setBool(target *bool, name string, errors *[]string) {
	if value, ok := os.LookupEnv(name); ok {
		parsed, err := strconv.ParseBool(value)
		if err != nil {
			*errors = append(*errors, fmt.Sprintf("%s must be a boolean", name))
			return
		}
		*target = parsed
	}
}

func setDuration(target *Duration, name string, errors *[]string) {
	if value, ok := os.LookupEnv(name); ok {
		parsed, err := time.ParseDuration(value)
		if err != nil {
			*errors = append(*errors, fmt.Sprintf("%s must be a duration", name))
			return
		}
		*target = NewDuration(parsed)
	}
}

func setByteSize(target *ByteSize, name string, errors *[]string) {
	if value, ok := os.LookupEnv(name); ok {
		parsed, err := ParseByteSize(value)
		if err != nil {
			*errors = append(*errors, fmt.Sprintf("%s must be a byte size", name))
			return
		}
		*target = NewByteSize(parsed)
	}
}

func setStringSlice(target *[]string, name string) {
	if value, ok := os.LookupEnv(name); ok {
		var output []string
		for _, part := range strings.Split(value, ",") {
			item := strings.TrimSpace(part)
			if item != "" {
				output = append(output, item)
			}
		}
		*target = output
	}
}
